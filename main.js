...
function updateVisitorStats() {
    ...
    const dateKey = 'wlwz-visitor-date';
    const todayKey = 'wlwz-visitor-today-count';
    const totalKey = 'wlwz-visitor-total-count';
    const sessionDateKey = 'wlwz-visitor-session-date';
    const now = new Date();
    ...
    let totalCount = 0;
    try {
        storedDate = (window.localStorage && window.localStorage.getItem(dateKey)) || '';
    } catch (e) {
        storedDate = '';
    }
    if (storedDate === todayStr) {
        try {
            if (window.localStorage) {
                const rawToday = window.localStorage.getItem(todayKey);
                todayCount = rawToday ? parseInt(rawToday, 10) || 0 : 0;
            }
        } catch (e) {
            todayCount = 0;
        }
    ...
    try {
        if (window.localStorage) {
            ...
            totalCount = rawTotal ? parseInt(rawTotal, 10) || 0 : 0;
        }
    } catch (e) {
        totalCount = 0;
    }
    ...
    try {
        if (window.sessionStorage) {
            sessionDate = window.sessionStorage.getItem(sessionDateKey) || '';
        }
    } catch (e) {
        sessionDate = '';
    }
    const shouldIncrement = sessionDate !== todayStr;
    if (shouldIncrement) {
        todayCount += 1;
        totalCount += 1;
        try {
            if (window.localStorage) {
                window.localStorage.setItem(dateKey, todayStr);
                window.localStorage.setItem(todayKey, String(todayCount));
                window.localStorage.setItem(totalKey, String(totalCount));
            }
        } catch (e) {
        }
        try {
            if (window.sessionStorage) {
                window.sessionStorage.setItem(sessionDateKey, todayStr);
            }
        } catch (e) {
        }
    }
    ...
    characterEntries.forEach(([id, char]) => {
        ...
        if (char.avatar) {
            ...
            img.onerror = () => {
                ...
                defaultImg.src = 'characters/avatars/portraits/default.png';
                defaultImg.onload = () => {
