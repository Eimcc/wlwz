...
function renderCharacterCards(characters) {
    ...
    characters.forEach(character => {
        ...
        card.innerHTML = `
            <a href="character.html?id=${character.id}" class="block">
...
function initCharacterPage() {
    ...
    if (selector) {
        ...
        Object.values(charactersData).forEach(char => {
            ...
            div.onclick = (e) => {
                e.preventDefault();
                const newUrl = `character.html?id=${char.id}`;
...
function loadCharacterData(id) {
    ...
    if (relationshipsContainer) {
        relationshipsContainer.innerHTML = character.relationships.map(rel => {
            ...
            return `
                <div class="flex items-center justify-between p-2 bg-white border border-gray-100 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer" onclick="window.location.href='character.html?id=${targetChar.id}'">
...
function updateActivitiesTimeDisplay() {
    ...
    if (timeEl) {
        const now = new Date();
        timeEl.textContent = getMingDateStr(now) + ' ' + getShichen(now);
    }
}
...
function generateCharacterLogsRange(character, context, startHoursAgo, endHoursAgoExclusive, stepHours) {
    ...
    for (let h = endHoursAgoExclusive + stepHours; h <= startHoursAgo; h += stepHours) {
        const time = new Date(now.getTime() - h * 60 * 60 * 1000);
        const entry = getCharacterActionAtTime(character, time, context.weather, context.location);
        logs.push(entry);
    }
    return logs;
}
...
function generateCharacterLogs(character, context, mode) {
    if (mode === '24h') {
        return generateCharacterLogsRange(character, context, 24, 0, 4);
    }
    if (mode === '7d') {
...
function getCharacterActionAtTime(character, time, weather, realLocation) {
    ...
    const timeLabel = getMingDateStr(time) + ' ' + getShichen(time);
    return {
        timeLabel: timeLabel,
